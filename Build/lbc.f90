...
!
      integer :: i, ibry, ie, ifield, is, lstr, lvar, status
...
!
      DO ifield=1,nLBCvar
        ...
        IF ((is.gt.0).and.(ie.gt.0).and.(ie.gt.is)) THEN
          ...
          IF (ie.le.0) ie=LEN_TRIM(line)      ! last attribute entry
          ...
          string(4)=Bstring(22:28)            ! north keyword
          DO ibry=1,4
            SELECT CASE (TRIM(string(ibry)))
              CASE ('Cha')
                IF (.not.S(ibry,ifield,ng)%Chapman) THEN
                  IF (Master) THEN
                    WRITE (stdout,20) B(ibry),                          &
     &                                TRIM(Vname(1,idBvar(ifield))),    &
     &                                TRIM(string(ibry)),               &
     &                                'S(',ibry,ifield,ng,')%Chapman',  &
     &                                S(ibry,ifield,ng)%Chapman,        &
     &                                TRIM(ncname)
                  END IF
                  exit_flag=5
...
!
      DO ifield=1,nLBCvar
        DO ibry=1,4
          IF (S(ibry,ifield,ng)%Chapman) THEN
            string(ibry)='Cha    '
          ...
          ELSE IF (S(ibry,ifield,ng)%gradient) THEN
            string(ibry)='Gra    '
          ELSE IF (S(ibry,ifield,ng)%mixed) THEN
            string(ibry)='Mix    '
...
!
      integer :: ibry, ng
      character (len=10) :: string(4,Ngrids)
!
!-----------------------------------------------------------------------
...
!
     ...
     &                     TRIM(string(4,ng))
        END IF
      END DO
 10   FORMAT (/,1x,a,t26,i2,t33,a,t45,a,t57,a,t69,a)
 20   FORMAT (t26,i2,t33,a,t45,a,t57,a,t69,a)
      RETURN

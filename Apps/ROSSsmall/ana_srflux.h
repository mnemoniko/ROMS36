...
!
!! svn $Id: ana_srflux.h 1169 2010-03-02 01:24:52Z kate $
!!======================================================================
...
!
     ...
     &                      GRID(ng) % lonr,                            &
     &                      GRID(ng) % latr,                            &
#if defined ALBEDO || defined ROSS
     &                      FORCES(ng) % cloud,                         &
     &                      FORCES(ng) % Hair,                          &
     &                      FORCES(ng) % Tair,                          &
#endif
...
!
      integer :: i, j
#if defined ALBEDO || defined DIURNAL_SRFLUX || defined ROSS
...
!
!  ROSS option: For the ROSS model, first compute the clear-sky 
...
!
!  Assume time is in modified Julian day.  Get hour and year day.
!  Note that the ROSS model is starting on 15 Sep (day=258)
!
...
!   use for now, 2nd version is old ROSS and should be tested)
!
!  (Now, 5/10/11, testing old ROSS method)
!
...
!!   &             (1.0_r8-0.62_r8*cloud(i,j)+0.0019_r8*sinzen) *       &
